...
app = modal.App("MoleculeX",image=evo2_image)
...
def brca1_example():
    ...
    if "plot" in result:
        ...
        img = mpimg.imread(BytesIO(plot_data), format='png')
...
def get_genome_sequence(position, genome:str, chromosome:str,window_size=8192):
    ...
    half_widnow = window_size // 2
    start = max(0, position -1- half_widnow)
    end = position - 1 + half_widnow + 1 # +1 for inclusive end
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to fetch sequence: {response.status_code} {response.text}")
    
    genome_data = response.json()

    if 'dna' not in genome_data:
        error = genome_data.get('error', 'Unknown error')
        raise Exception(f"Error fetching sequence(UCSC API ERROR): {error}")
    
    sequence = genome_data.get('dna', '').upper()

    expected_length = end - start 
    if len(sequence) != expected_length:
        print(f"Warning: Expected sequence length {expected_length}, got {len(sequence)}")


    print(f"Loaded genome sequence successfully.(Lenght:{len(sequence)})")   
    return sequence, start  


def analyze_variant_sequence(relative_pos_in_window, reference, alternative, window_seq, model):
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_position:int,alternative:str,genome:str,chromosome:str):
        ...
        print("Alternative allele:", alternative)
        ...
        relative_pos = variant_position - 1-seq_start # Convert to 0-indexed position relative to the fetched sequence
        print(f"Relative position in sequence: {relative_pos}")

        if relative_pos < 0 or relative_pos >= len(window_seq):
            raise ValueError(f"Variant position {variant_position} is out of bounds for the fetched sequence.")

        reference = window_seq[relative_pos]
        print("Reference is:", reference) 

        #Analyze variant sequence
        result = analyze_variant_sequence(
...
def main():
    evo2Modal = Evo2Model()
    result = evo2Modal.analyze_single_variant.remote(
